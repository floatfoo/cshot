...
static struct argp_option options[] = {
    ...
    { "delay", 'D', "sec", 0, "Delay before the actual screenshot (in sec)",
      -1 },
};
    ...
    if (delay_str)
        {
            ...
            if (errno == EINVAL)
                ...
                {
                    fprintf (stderr, "Delay argument error -- error parsing "
                                     "given argument.");
                    return 1;
                }
            else
...
main (int argc, char **argv)
{
    ...
    if (getenv ("WAYLAND_DISPLAY"))
        return take_screenshot (arguments.output_path, w_get_bitmap) == 0 ? 0
                                                                          : 1;
    else
        return take_screenshot (arguments.output_path, x_get_bitmap) == 0 ? 0
                                                                          : 1;
};
